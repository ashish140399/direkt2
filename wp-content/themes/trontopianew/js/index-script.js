...
$(document).ready(async function() {
    ...
    async function tronLoginCheck() {
        ...
        try {
            if (!window.tronWeb)  { //throw "You must install tronlink extension";
             ...
             //console.log("1");
             setInterval(getindexStatusData,3000);
             }
   ...
   function getindexStatusData(){
       $.get(API_URL + "indexStatusData.php", function(data, status,xhr){
                if(xhr.status==200){
                        data = JSON.parse(data);
        
                        if(data.result==true){
                            var topiaMinted = data.topiaMinted;
                            var totalRewards = data.totalRewards;
                            var unBurnedVouchers = data.unBurnedVouchers;
                            var diamondrewards = data.diamondrewards;
                            $("#minedToken2").html(topiaMinted);
                            $("#dividendsTotal").html(totalRewards);
                            $("#vouchersMined").html(unBurnedVouchers);
                            $("#diamondsDividendsTotal").html(diamondrewards);
                            $("#stageNumber").html(data.stage);
                            $("#stageTRX").html(data.trx);
                            $("#stageNumber2").html(data.nextStage);
                            $("#stageTRX2").html(data.nextTrx);
                            $("#ante-jd-text").html(data.totalMintedToken + ' / 1000000');
                            $('#topia-progress').css('cssText',"width: "+ data.topia_progress + "% !important;");
                            $("#dividends").html(data.totalRewards);
                            var trxAccumulated = (data.totalRewards*10)/100;
                            $("#accumulatedTRX").html(trxAccumulated.toFixed(2));
                            $("#totalFrozenTopia").html(data.totalFrozenTopia);
                            $("#diamond-jd-text").html(data.totalDiamonds+' / '+data.maxSupply); 
                            $('#diamond-progress').css('cssText',"width: "+ data.diamond_progress + "% !important;");
                            $("#diamondsDividends").html(data.diamondrewards);
                            $("#totalDiamondsFrozen").html(data.totalDiamondsFrozen);
                            $("#shares-jd-text").html(data.shareTotalSupply+' / 1000000000');  
                            var share_progress = (data.shareTotalSupply*100)/1000000000;
                            $('#shares-progress').css('cssText',"width: "+ share_progress + "% !important;");
   
                            $("#topiaSharesText").html(data.topiaShare);
                            $("#shareSharesText").html(data.topiaShareShare);
                            $("#usdtSharesText").html(data.usdtShare);
                            $("#twxSharesText").html(data.twxShare);
                            $("#fragSharesText").html(data.fragShare);
                            $("#bnkrSharesText").html(data.bnkrShare);
                            $("#bttSharesText").html(data.bttShare);
                            $("#swgSharesText").html(data.swgShare);
 ...
 $('#withdrawUnFrozenSharesBtn').click(async function(){
     var result = shareContractInstance.withdrawUnfrozenShares().send({
        shouldPollResponse: true,
        //feeLimit: 1000000000,
        feeLimit: 5000000,
        callValue: 0,
