                    ...
                    if(data.result==true){
					        ...
					        if(user1==global.userAddressHex && global.uniqueBetId==_uniqueBetId){
                                    ...
                                    const result = await luckyOneContractInstance.calculateBetResultWithBlockHash(
                                   	...
                                   	console.log('lucky number' + result._randomNumber);
...
function randomNumber_to_digitToCount_and_largestDigitAmount_and_jackpotType(randomNumber)
{
    ...
    async function trxTopiaFunc(){
        var voucherMintRate = await voucherContractInstance.mintingBasePrice(luckyOneContractAddress).call();
       ...
       console.log('unique string : '+ uniqueString); */
  	    try
  	    {
            console.log('finishbet unique id : ' +finishBet_uniqueBetId);
                ...
                var result = await luckyOneContractInstance.finishBet_and_startBet(finishBet_blockNumber,finishBet_gambler, finishBet_trx, finishBet_userSeed,finishBet_uniqueBetId, REFID, betAmt, seed, uniqueString ).send({
