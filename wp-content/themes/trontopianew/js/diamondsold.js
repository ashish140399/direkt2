...
const diamondDividendContractAddress    =  'TQU25EHPd6ZqdDgYD7p4oQ3uzBYsVzgufE';
...
$('#withdrawDiamondsDividendBtnOld').click(async function(){
   var result = await DivdiamondContractInstance.withdrawDividendsEverything().send({
...
//unfreeze diamonds 
    $('#unfreezeDiamondsBtn').click(async function(){
        ...
        if(diamonds > 0){
            var result = await diamondContractInstance.unfreezeDiamonds().send({
                shouldPollResponse: true,
                feeLimit: 5000000,
                callValue: 0,
                from: global.userAddress
       ...
       // console.log('vouchers available : ' + vouchersAvailable);
        ...
        var isMobile = /iPhone|iPad|iPod|Android/i.test(navigator.userAgent);
         ...
         if(diamonds>0 && diamonds<=diamondsAvailable){
            ...
            var result = await diamondContractInstance.freezeDiamond(diamonds).send({
                shouldPollResponse: true,
                //feeLimit: 1000000000,
                feeLimit: 5000000,
                callValue: 0,
