...
if($result){
    ...
    $sqluser_token = "select sum(bet) total from ( select bet from roll where timestamp>='".$timestamp."' AND user='".$userAddress."'  union all select bet from sidebetrolls where timestamp>='".$timestamp."' AND user='".$userAddress."' union all select bet from roll_classicDice where timestamp >".$timestamp." AND user='".$userAddress."') t";
    ...
    if($resuser_token && mysqli_num_rows($resuser_token)>0){
        ...
        $myTotalMinted = $row_user_token['total']/833;
    }else{
        $myTotalMinted = 0;
    }
